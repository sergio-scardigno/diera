...
trait HasRoleAndPermission
{
    ...
    {   
        return ($role = $this->getRoles()->where('sistema', '=','diera')->sortBy('level')->first() and $this->activo) ? $role->level : 0;
        // return ($role = $this->getRoles()->sortByDesc('level')->first()) ? $role->level : 0;
    }

    /**
     * Get role level of a user en un sistema.
     *
     * @return int
     */
    public function level_sistema($sistema)
    {
        
        return ($role = $this->getRoles()->where('sistema', '=',$sistema)->sortBy('level')->first() and $this->activo) ? $role->level : 0;
    }
