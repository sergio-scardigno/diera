 ...
 <div id="app">
   <nav class="navbar navbar-expand-md navbar-light navbar-laravel">
         ...
         <ul class="navbar-nav mr-auto">            
            ...
            <div class="btn btn-primary dropdown-menu" aria-labelledby="navbarDropdown">
               ...
               @foreach ($periodo_en_carga[0] as $element =>$u)
                  @if ($u->momento !='C')
                   <a class="btn btn-primary dropdown-item"href="{{ url('/localizacion', $u->id_periodo) }}">{{$u->periodo}}</a>
                   @endif  
               @endforeach    
            ...
            <li class="nav-item dropdown">
                ...
                <div class="btn btn-primary dropdown-menu" aria-labelledby="navbarDropdown">
                    ...
                    @foreach ($periodo_en_carga[0] as $element =>$u)                             
                        @if ($u->momento !='C')                           
                          <a class="btn btn-primary dropdown-item"href="{{ url('/estadistica', $u->id_periodo) }}">{{$u->periodo}} </a>
                        @endif
                    @endforeach    
                    <div class="dropdown-divider"></div>              
                    <div class="dropdown-header"><b> {{ __('Consulta de Relevamientos') }}</b></div>
                    ...
                    @foreach ($periodo_en_carga[0] as $element =>$u)     
                        @if ($u->momento !='C')                                                        
                          <a class="btn btn-primary dropdown-item"href="{{ url('/estadisticacarga', $u->id_periodo) }}">{{$u->periodo}} </a>
                        @endif
                    @endforeach    
                </div>
                @endrole
            ...
            <li class="nav-item dropdown">
                ...
                <div class="btn btn-primary dropdown-menu" aria-labelledby="navbarDropdown">                           
                    <div class="dropdown-header"><b> {{ __('Relevamientos en carga') }}</b></div>  
                 @foreach ($periodo_en_carga[0] as $element =>$u)                
                    <a class="btn btn-primary dropdown-item"href="{{ url('/pservicio', $u->id_periodo) }}"> Altas y Bajas Servicio Educativo {{$u->periodo}}</a>    
                 @endforeach             
            ...
            @endrole
            @role('admin|supervisor')  <!-- Procesos -->
            <li class="nav-item dropdown">
                <a class="btn btn-primary dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ __('Procesos') }}
                </a>
            ...
            <li class="nav-item dropdown">
                ...
                @foreach ($periodo_en_carga[0] as $element =>$u)     
                    @if ($u->momento !='C')                                                                    
                        <a class="btn btn-primary dropdown-item"href="{{ url('/mailing', $u->id_periodo) }}">{{ __('Envio e-mail ') }}{{$u->periodo}} </a>
                    @endif
                @endforeach    
            ...
            <li class="nav-item dropdown">
                ...
                <div class="btn btn-primary dropdown-menu" aria-labelledby="navbarDropdown">       
                  <a class="btn btn-primary dropdown-item"href="{{ url('/usuarios') }}">{{ __('Usuarios') }}</a>
                  <a class="btn btn-primary dropdown-item"href="{{ url('/periodo') }}">{{ __('Periodos') }} </a>
               </div>
            </li>
    ...
    <a class="navbar-brand" href="#">                    
        <img src="{{ asset('images/logo_gba_footer.svg') }}" width="120">
    </a>
  </nav>
  </div>
  <main class="py-1">     
