...
<div class="container-fluid"> {{--  Container  --}}
    ...
    <div class="card-header separa_datos"> {{-- Separa datos --}}
        <b>{{$formulario->nombre}} - {{$formulario->codigo_jurisdiccional}} - {{$formulario->cueanexo}} -
            {{$formulario->esc_nombre}} </b>
    </div> {{-- Fin Separa datos --}}

    {!!Form::model($formulario,['id'=>'formulario', 'url'=>['grabarformulario/'],'method'=>'POST'])!!}
    {!! csrf_field() !!}
    <input id="{{$formulario->id_localizacion_periodo}}" name="id_localizacion_periodo" type="number"
        value="{{$formulario->id_localizacion_periodo}}" hidden>
        ...
        <div class="panel-heading-custom panel-heading heading">{{--  panel-heading-custom panel-heading heading --}}
            <div class="panel-body"> {{-- panel-body --}}
                ...
                <div class="table-responsive">
                    @if (Auth::user()->level() < 6 and $formulario->c_estado_formulario <100 and $u->
                            c_estado_cuadro!=87)
                            ...
                            $(document).ready(function() {
                                inhabilitar_cuadro($u - > numero);
                            });
                            </script>"; @endphp
                            <input id="cuadros[{{$u->numero}}]" name="cuadros[{{$u->numero}}]"
                                class="btn btn-primary focusNext" type="checkbox" checked
                                onclick="inhabilitar_cuadro({{$u->numero}});">
                            @else
                            @php echo "<script>
                            $(document).ready(function() {
                                inhabilitar_cuadro_tipo4($u - > numero);
                            });
                            </script>"; @endphp
                            <input id="cuadros[{{$u->numero}}]" name="cuadros[{{$u->numero}}]"
                                class="btn btn-primary focusNext" type="checkbox" checked
                                onclick="inhabilitar_cuadro_tipo4({{$u->numero}});">
                            @endif
                            <label for="cuadros[{{$u->numero}}]"> Cuadro sin Información&nbsp;&nbsp;</label>
                            @else
                            @if($u->c_tipo_cuadro!=4)
                            <input id="cuadros[{{$u->numero}}]" name="cuadros[{{$u->numero}}]"
                                class="btn btn-primary focusNext" type="checkbox"
                                onclick="inhabilitar_cuadro({{$u->numero}});">
                            @else
                            ...
                            @endif
                            @if($u->c_tipo_cuadro!=4)
                            <button id="agregar0{{$u->numero}}" type="button" class="btn btn-primary"
                                onclick="agregar_ceros({{$u->numero}},{{$u->c_tipo_cuadro}});"
                                title="Agregar cero/s (0) en las celdas vacias"><i class="fa fa-plus"></i> Agregar 0
                            </button> &nbsp;&nbsp;
                            <button id="quitar0{{$u->numero}}" type="button" class="btn btn-primary"
                                onclick="quitar_ceros({{$u->numero}},{{$u->c_tipo_cuadro}});"
                                title="Borrar cero/s (0) de las celdas"> <i class="fa fa-times"></i> Borrar 0 </button>
                            &nbsp;&nbsp;
                            @endif

                            @if($u->c_tipo_cuadro==2 or $u->c_tipo_cuadro==3 and Auth::user()->level() < 6 ) <button
                                id="agregar_fila{{$u->numero}}" type="button" class="btn btn-primary"
                                onclick="agregar_fila({{$u->numero}},{{$u->c_tipo_cuadro}});"
                                title="Agregar una nueva fila en el cuadro"> <i class="fa fa-table"></i> Nueva
                                fila</button> &nbsp;&nbsp;
                                @endif
                                <button id="limpiar_todo{{$u->numero}}" type="button" class="btn btn-primary"
                                    onclick="vaciar_cuadro({{$u->numero}},{{$u->c_tipo_cuadro}});"
                                    title="Limpiar todas las celdas del cuadro"> <i class="fa fa-eraser"></i> Limpiar
                                    cuadro </button> &nbsp;&nbsp;
                                @endif {{-- (Auth::user()->level()  --}}
                                <table id="tabla{{$u->numero}}" aria-describedby="tabla{{$u->numero}}Help"
                                    class="table table-bordered table-condensed cuadro">
                                    <tbody>
                                        @for ($fil=0; $fil<count($u->celdas); $fil++)
                                            @if($u->c_tipo_cuadro==2)
                                            @if ($u->celdas[$fil+1][1]->editable) @php
                                            $tot_fil_editables=$tot_fil_editables+1; @endphp @endif
                                            @if ($u->celdas[$fil+1][1]->titulo) @php
                                            $total_fil_titulos=$total_fil_titulos+1; @endphp @endif
                                            ...
                                            @endif
                                            <tr id="cuadros[{{$u->numero}}][{{$fil+1}}]"
                                                name="cuadros[{{$u->numero}}][{{$fil+1}}]">
                                                @for ($col=0;$col<count($u->celdas[$fil+1] );$col++)
                                                    @if ($u->celdas[$fil+1][$col+1]->tipo_dato !='null' )
                                                    @if ($u->celdas[$fil+1][$col+1]->titulo)
                                                    <th colspan={{$u->celdas[$fil+1][$col+1]->colspan}}
                                                        rowspan={{$u->celdas[$fil+1][$col+1]->rowspan}}
                                                    ...
                                                    <td class="habilitada">
                                                        @endif
                                                        @elseif ($u->celdas[$fil+1][$col+1]->c_categoria_consistencia
                                                        ==2)
                                                    ...
                                                    <td class="error">
                                                        @endif
                                                        @php $id=$id+1; @endphp
                                                        @if (Auth::user()->level() < 6 and $formulario->
                                                            c_estado_formulario <100) @if ($u->
                                                                celdas[$fil+1][$col+1]->tipo_dato =='number')
                                                                <div align="center">

                                                                    <!-- prueba -->
                                                                    <input id="@php echo $id;@endphp"
                                                                        tabindex="@php echo $id;@endphp"
                                                                        name="cuadros[{{$u->numero}}][{{$fil+1}}][{{$col+1}}]"
                                                                        type="{{$u->celdas[$fil+1][$col+1]->tipo_dato}}"
                                                                        value="{{$u->celdas[$fil+1][$col+1]->valor }}"
                                                                        min="0" style="text-align:right;"
                                                                        class="form-control focusNext"
                                                                        title="{{$u->celdas[$fil+1][$col+1]->msg}}">


                                                                </div>
                                                                @elseif ($u->celdas[$fil+1][$col+1]->tipo_dato
                                                                =='combobox' )
                                                                {{-- combobox       --}}
...
<input type="text" name="answer" id="answerInput-hidden">  --}} -->
                                                                ...
                                                                <!-- Prueba codigo a mejorar -->

                                                                <!-- <select name="cuadros[{{$u->numero}}][{{$fil+1}}][{{$col+1}}]" id="@php echo $id;@endphp" tabindex="@php echo $id;@endphp"
                                class="form-control focusNext" title="{{$u->celdas[$fil+1][$col+1]->msg}}" data-select-id="mi-select">
                                @foreach ($opciones_combos->wherein('id_combo', [$u->celdas[$fil+1][$col+1]->valor_inicial,0]) as $element =>$op)
                                  @if ($u->celdas[$fil+1][$col+1]->valor==$op->id_opciones_combos)
                                ...
                                @endforeach
                                                                ...
                                                                <select
                                                                    name="cuadros[{{$u->numero}}][{{$fil+1}}][{{$col+1}}]"
                                                                    id="@php echo $id;@endphp"
                                                                    tabindex="@php echo $id;@endphp"
                                                                    class="form-control focusNext"
                                                                    title="{{$u->celdas[$fil+1][$col+1]->msg}}"
                                                                    data-select-id="mi-select">

                                                                    @foreach ($opciones_combos->wherein('id_combo',
                                                                    [$u->celdas[$fil+1][$col+1]->valor_inicial,0]) as
                                                                    $element =>$op)
                                                                    @if
                                                                    ($u->celdas[$fil+1][$col+1]->valor==$op->id_opciones_combos)
                                                                    <option value="{{$op->id_opciones_combos}}"
                                                                        selected="selected">{{$op->descripcion}}
                                                                    </option>
                                                                    @else
                                                                    <option value="{{$op->id_opciones_combos}}">
                                                                ...
                                                                <input id="@php echo $id;@endphp"
                                                                    ...
                                                                    name="cuadros[{{$u->numero}}][{{$fil+1}}][{{$col+1}}]"
                                                                    type="{{$u->celdas[$fil+1][$col+1]->tipo_dato}}"
                                                                    checked title="{{$u->celdas[$fil+1][$col+1]->msg}}">
                                                                @endif
                                                                {{-- radio  --}}
                                                                @elseif ($u->celdas[$fil+1][$col+1]->tipo_dato
                                                                =='radio')
                                                                @php
                                                                $opciones = explode(';',
                                                                $u->celdas[$fil+1][$col+1]->valor_inicial);
                                                                @endphp
                                                                ...
                                                                <label>
                                                                    <input id="{{$id}}"
                                                                        ...
                                                                        type="{{$u->celdas[$fil+1][$col+1]->tipo_dato}}"
                                                                        @if
                                                                        ($loop->iteration==$u->celdas[$fil+1][$col+1]->valor)
                                                                    checked
                                                                    @endif
                                                                ...
                                                                <div align="center">
                                                                    <input id="@php echo $id;@endphp"
                                                                        ...
                                                                        type="{{$u->celdas[$fil+1][$col+1]->tipo_dato}}"
                                                                        value="{{$u->celdas[$fil+1][$col+1]->valor }}"
                                                                        min="0" align="right" style="text-align:right;"
                                                                        readonly
                                                                        title="{{$u->celdas[$fil+1][$col+1]->msg}}"
                                                                        class="form-control focusNext">
                                                                </div>
                                                                @elseif ($u->celdas[$fil+1][$col+1]->tipo_dato
                                                                =='combobox' )
                                                                <select id="@php echo $id;@endphp"
                                                                    name="cuadros[{{$u->numero}}][{{$fil+1}}][{{$col+1}}]"
                                                                    readonly
                                                                    title="{{$u->celdas[$fil+1][$col+1]->msg}}">
                                                                    @foreach ($opciones_combos->where('id_combo',
                                                                    $u->celdas[$fil+1][$col+1]->valor_inicial) as
                                                                    $element =>$op)
                                                                    @if
                                                                    ($u->celdas[$fil+1][$col+1]->valor==$op->id_opciones_combos)
                                                                    <option value="{{$op->id_opciones_combos}}"
                                                    ...
                                                    @if (Auth::user()->level() < 6 and $formulario->c_estado_formulario
                                                        <100) {{-- Boton eliminar fila cuadro tipo 2 y 3 --}} @if
                                                            ($fil+1<=count($u->celdas))
                                                            ...
                                                            <td style="display: none;">
                                                                <button type="button" class="btn btn-danger"
                                                                    onclick="eliminar_fila({{$u->numero}},{{$fil+1}});"><i
                                                                        class="fa fa-trash-o"></i> </button>
                                                            </td>
                                                            @elseif ($tot_fil_editables > 1 and $tot_fil_editables <
                                                                $total_filas ) @if (Auth::user()->level() < 6) <td>
                                                                    <button type="button" class="btn btn-danger"
                                                                        onclick="eliminar_fila({{$u->numero}},{{$fil+1}});"><i
                                                                            class="fa fa-trash-o"></i> </button>
                                                                    </td>
                                ...
                                <small id="tabla{{$u->numero}}Help"
                                    class="form-text text-muted"><b>{{ $u->indicaciones }}</b></small>

                                @if ($u->msg_err != [])
                                <!-- <div  class="navbar navbar-fixed-bottom justify-content-center">  -->
                                <div class="alert alert-danger" role="alert">
                                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                                    <strong><i class='fa fa-times-circle'></i> Por favor corrige los siguentes
                                        errores</strong>
                                    <ul>
                                ...
                                <div class="alert alert-warning">
                                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                                    <strong><i class='fa fa-warning'></i> Por favor verifique las siguentes
                                        advertencias</strong>
                                    <ul>
            ...
            @if ($formulario->c_estado_formulario <100 && Auth::user()->level() < 6 && $periodo->en_carga==true )
                ...
                'value'=>'grabar', 'class' => 'btn btn-primary', 'title'=>'Grabar cuadros'))}}
                    ...
                    <a class="btn btn-primary"
                        href="{{ route('formulario',[$formulario->id_localizacion_periodo, $start]) }}">Salir de la
                        Planilla</a>
        </ul>
...
<!-- Asegúrate de incluir primero jQuery, luego Bootstrap y finalmente Bootstrap Select -->
...
<!-- Inicializar Select2 con las opciones deseadas -->
<script>
//   $(document).ready(function() {
//   // Inicializa Select2 en todos los elementos SELECT con el atributo "data-select-id" igual a "mi-select".
//   $('select[data-select-id="mi-select"]').select2({
//     minimumInputLength: 3,
//     language: {
//       inputTooShort: function () {
//         return "Ingresa más de 3 caracteres para realizar la búsqueda...";
//       },
//       noResults: function() {
