...
$(".pagination").on("click", "li", function() {
    if (modified) {
        return confirm(
            "Los cambios implementados no fueron guardados ¿Desea cambiar de página todas formas?"
        );
    }
});
...
function inhabilitar_cuadro(cuadro) {
    //&& !$(this).is('[readonly]')
    table = "#tabla" + cuadro;
    nombre = "[" + cuadro + "]";
    if ($("input[name='cuadros" + nombre + "']").is(":checked")) {
        $(table).each(function(tr_idx, tr_elem) {
            $tr = $(tr_elem);
            $tr.find("input, select, textarea, checkbox, radio").each(function(
                nColumnas,
                i_elem
            ) {
                if (
                    ...
                    $(table)
                        .find("input,button,textarea,select, checkbox, radio")
                        .removeAttr("disabled");
                    $(this).attr("class", "habilitada");
                    $("input[name='cuadros" + nombre + "']").prop(
                        "checked",
                        false
                    );
                    ...
                    var html =
                        "<div class='alert alert-info'><button type='button' class='close' data-dismiss='alert'>&times;</button>  <strong><i class='fa fa-info-circle fa-2x'> </i> El cuadro debe estar vacío para poder marcarlo sin información</strong></div>";
...
function inhabilitar_cuadro_tipo4(cuadro) {
    //&& !$(this).is('[readonly]')
    table = "#tabla" + cuadro;
    nombre = "[" + cuadro + "]";
    if ($("input[name='cuadros" + nombre + "']").is(":checked")) {
        $(table).each(function(tr_idx, tr_elem) {
            $tr = $(tr_elem);
            $tr.find("input, select, textarea, checkbox").each(function(
                nColumnas,
                i_elem
            ) {
                if (
                    ...
                    $(table)
                        .find("input,button,textarea,select, checkbox")
                        .removeAttr("disabled");
                    $(this).attr("class", "habilitada");
                    $("input[name='cuadros" + nombre + "']").prop(
                        "checked",
...
// Funcion eliminar_fila Trabajando 21/03/24

// function eliminar_fila(cuadro, fila) {
//   // Flag para indicar si se ha modificado algo
//   let modified = true;

//   // Selector de la tabla y nombre de los elementos de formulario
//   const table = `#tabla${cuadro}`;
//   const inputs = `cuadros[${cuadro}]`;

//   // Se obtiene el índice de la fila a eliminar
//   const indiceFila = parseInt(fila) - 1;

//   // Se elimina la fila correspondiente
//   $(`${table} tr:eq(${indiceFila})`).remove();

//   // Se actualizan los nombres y IDs de las filas restantes
//   $(`${table} tr`).each(function(nfila, tr) {
//       nfila++;
//       $(this).attr({
//           'name': `${inputs}[${nfila}]`,
//           'id': `${inputs}[${nfila}]`
...
//       // Se actualiza el evento onclick de los botones
//       $(this).find('button').attr('onclick', `eliminar_fila(${cuadro}, ${nfila})`);

//       // Se actualizan los nombres de los elementos de formulario
//       $(this).find('input, select, textarea, combobox').each(function(nColumnas) {
//           nColumnas++;
//           $(this).attr('name', `${inputs}[${nfila}][${nColumnas}]`);
//       });
//   });

//   // Se actualiza el número total de filas
//   const nFilas = $(`${table} tr`).length;

//   // Se actualizan los nombres de los elementos de la última fila
//   $(`${table} tr:last input, ${table} tr:last select, ${table} tr:last textarea, ${table} tr:last combobox`).each(function(nColumnas) {
//       nColumnas++;
//       $(this).attr('name', `${inputs}[${nFilas}][${nColumnas}]`);
//   });

//   // Se actualizan los atributos ID y tabindex de los elementos de formulario
//   let id = 0;
//   $('input, select, textarea, combobox').not('[readonly]').each(function() {
//       id++;
//       $(this).attr({
//           'id': id,
//           'tabindex': id
...
// Funcion eliminar_fila ORIGINAL
...
// Funcion eliminar_fila Trabajando 11/04/24
function eliminar_fila(cuadro, fila) {
    let table = "#tabla" + cuadro;
    let inputs = 'cuadros[' + cuadro + ']';
    ...
    // Reindexar filas, manteniendo los índices de las columnas internas constantes
    $(table + ' tr').each(function (index, element) {
        if (!$(this).hasClass('fila-totales')) {
            $(this).find('input, select, textarea, combobox').each(function () {
                // Extraer el actual índice de columna del nombre actual
                let currentName = $(this).attr('name');
                let match = currentName.match(/\[(\d+)\]\[(\d+)\]$/);
                if (match) {
                    let colIndex = match[2]; // Índice de columna permanece constante
                    $(this).attr('name', inputs + '[' + (index + 1) + '][' + colIndex + ']');
                }
            });

            $(this).find('button').attr('onclick', 'eliminar_fila(' + cuadro + ',' + (index + 1) + ')');
        }
    });
}
...
// Funcion agregar_fila ORIGINAL NO FUNCIONA CON SELECT2
...
// Funcion agregar_fila FUNCIONA CON ERRORES
...
//     if (c_tipo_cuadro == 2) {
//         let primera_fila_editable = parseInt(
//             table.rows[0].firstElementChild.getAttribute("rowspan")
//         );
//         let nColumnas_editables =
//             table.rows[primera_fila_editable].cells.length;
...
//         table.rows[primera_fila_editable]
...
//             .forEach((elem, nColumnas) => {
//                 elem.setAttribute(
//                     "name",
//                     inputs + "[" + nFilas + "][" + nColumnas + "]"
//                 );
...
//             .insertAdjacentElement("beforebegin", newRow);
...
//         ultimaFila
//             .querySelectorAll("input, select, textarea, combobox")
//             .forEach((elem, nColumnas) => {
//                 nColumnas = parseInt(nColumnas) + 1;
//                 elem.setAttribute(
//                     "name",
...
//         let name_atri = document
//             .querySelector("input:enabled:not([readonly]):first-child")
//             .getAttribute("name");
//         let id = 0;
...
// Funcion agregar_fila TRABAJANDO (similar a la original)
...
function agregar_fila(cuadro, c_tipo_cuadro) {
    ...
    if (c_tipo_cuadro==2) {
     ...
     $(table + ' tr:eq('+primera_fila_editable+')').clone().each(function(tr_idx, tr_elem) {
      ...
      $(table + ' tr:last select').each(function() {
        new Choices(this, {
            searchEnabled: false, // O cualquier configuración de Choices.js que necesites
            itemSelectText: '', // Personaliza según necesites
        });
    });
...
document.addEventListener("keydown", function(event) {
    ...
    if (
        letrasSalto.includes(event.key.toLowerCase()) ||
        event.keyCode === 9 ||
        event.keyCode === 13
    ) {
        event.preventDefault();
        let inputsFocus = document.querySelectorAll(
            "input.focusNext:not(:disabled):not([readonly])" +
...
function vaciar_cuadro(cuadro) {
    var check = confirm(
        "Va a eliminar todos los datos del cuadro. Esta seguro?"
    );
    if (check) {
        modified = true;
        inputs = "cuadros[" + cuadro + "]";
